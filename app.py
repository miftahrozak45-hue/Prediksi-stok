...
# fallback kalau secrets belum diisi
if not USERS:
    USERS = {
...
# PSO: viewer kita kunci
if role == "viewer":
    st.sidebar.write("🔒 PSO dikunci untuk viewer")
    run_pso = False
else:
    run_pso = st.sidebar.toggle("Jalankan PSO untuk optimasi boundaries", value=False)

num_intervals = st.sidebar.slider("Jumlah interval fuzzy", 5, 12, 8)
...
# 👉 Tambahkan opsi Apriori di sidebar
st.sidebar.header("🧩 Analisis Bundling (Apriori)")
run_apriori = st.sidebar.toggle("Jalankan Apriori / bundling", value=True, help="Perlu kolom: REFASALSEP dan OBAT")
min_support_apr = st.sidebar.slider("Min support Apriori", 0.01, 0.5, 0.02, step=0.01)
...
# =====================
# 7. APRIORI / BUNDLING OBAT
# =====================
df_bundling = pd.DataFrame()

if run_apriori:
    # cek kolom
    if {"REFASALSEP", "OBAT"}.issubset(df_raw.columns):
        st.subheader("🧩 Rekomendasi Bundling Obat (Apriori)")
        # bentuk basket: 1 baris = 1 SEP, kolom = obat
        # easiest: pakai crosstab
        basket = pd.crosstab(df_raw["REFASALSEP"], df_raw["OBAT"])
        # ubah ke 0/1
        basket = basket.applymap(lambda x: 1 if x > 0 else 0)

        # import di sini supaya aman kalau mlxtend belum ada
        ...
        if not freq_itemsets.empty:
            rules = association_rules(
                freq_itemsets,
                metric="lift",
                min_threshold=min_lift_apr
            )
            # ambil hanya aturan yang antecedents-nya 1 obat saja
            rules_1 = rules[rules["antecedents"].apply(lambda x: len(x) == 1)]
            # bentuk: OBAT → list bundling terurut
            bundling_rows = []
            ...
            df_bundling = pd.DataFrame(bundling_rows).sort_values("JUMLAH_REKOMENDASI", ascending=False)
            st.dataframe(df_bundling)
        else:
            st.info("Tidak ada frequent itemset pada support tersebut. Turunkan min_support.")
    else:
        st.warning("Kolom 'REFASALSEP' dan/atau 'OBAT' tidak ditemukan di data upload, sehingga Apriori tidak dijalankan.")
...
def to_excel_bytes(df_dict):
    ...
    with pd.ExcelWriter(buf, engine="openpyxl") as xw:
        ...
        # kalau kosong tetap bikin workbook
    buf.seek(0)
    return buf
